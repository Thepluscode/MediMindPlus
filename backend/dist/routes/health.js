...
const MonitoringManager_1 = require("../monitoring/MonitoringManager");
const router = (0, express_1.Router)();
exports.healthRoutes = router;
// Initialize managers for health checks
const databaseManager = new DatabaseManager_1.DatabaseManager();
const redisManager = new RedisManager_1.RedisManager();
const monitoringManager = new MonitoringManager_1.MonitoringManager();
/**
 * @swagger
 * /health:
 *   get:
...
router.get('/', (req, res) => {
    ...
    logger_1.enhancedLogger.debug('Basic health check requested', {
        ip: req.ip,
 ...
 */
router.get('/detailed', healthCheck_1.healthCheckHandler);
/**
 * @swagger
 * /health/ready:
 *   get:
...
router.get('/ready', async (req, res) => {
    try {
        ...
        if (failedChecks.length === 0) {
            ...
            logger_1.enhancedLogger.warn('Readiness check failed', {
        ...
        logger_1.enhancedLogger.error('Readiness check error', {
            error: error.message
        });
        res.status(503).json({
            status: 'error',
...
router.get('/metrics', async (req, res) => {
    try {
        ...
        logger_1.enhancedLogger.error('Metrics endpoint error', {
            error: error.message
        });
        res.status(500).json({
...
router.get('/redis', async (req, res) => {
    ...
    try {
        ...
        const responseTime = Date.now() - startTime;
        logger_1.enhancedLogger.error('Redis health check failed', {
            error: error.message,
            responseTime
        });
        res.status(503).json({
