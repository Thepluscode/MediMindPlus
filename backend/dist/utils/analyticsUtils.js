...
exports.ValidationUtils = exports.DataProcessingUtils = exports.TimeSeriesUtils = exports.StatisticsUtils = void 0;
/**
...
class StatisticsUtils {
     ...
     */
    static calculateZScores(values) {
     ...
     */
    static calculateExponentialMovingAverage(values, alpha = 0.3) {
        if (alpha <= 0 || alpha > 1) {
...
class TimeSeriesUtils {
     ...
     */
    static detectSeasonality(values, period = 7) {
     ...
     */
    static removeOutliers(values, multiplier = 1.5) {
     ...
     */
    static interpolateMissingValues(values) {
        const result = [...values];
...
class DataProcessingUtils {
     ...
     */
    static groupByMetric(dataPoints) {
     ...
     */
    static resampleData(dataPoints, intervalMinutes = 60) {
     ...
     */
    static calculateDataQuality(dataPoints) {
        if (dataPoints.length === 0) {
            return { completeness: 0, consistency: 0, accuracy: 0, overall: 0 };
        }
        // Completeness: ratio of non-null values
        const nonNullCount = dataPoints.filter(point => point.value !== null && point.value !== undefined && !isNaN(point.value)).length;
    ...
    static calculateAccuracyScore(dataPoints) {
        ...
        dataPoints.forEach(point => {
            ...
            switch (point.metric.toLowerCase()) {
                case 'heart_rate':
                    if (point.value >= 30 && point.value <= 220)
                        validCount++;
                    break;
                case 'blood_pressure_systolic':
                    if (point.value >= 70 && point.value <= 250)
                        validCount++;
                    break;
                case 'blood_pressure_diastolic':
                    if (point.value >= 40 && point.value <= 150)
                        validCount++;
                    break;
                case 'temperature':
                    ...
                    break;
                case 'steps':
                    if (point.value >= 0 && point.value <= 50000)
                        validCount++;
                    break;
                case 'sleep_duration':
                    if (point.value >= 0 && point.value <= 24)
                        validCount++;
                    break;
                default:
                    // For unknown metrics, assume valid if it's a reasonable number
                    if (point.value >= -1000 && point.value <= 1000000)
                        validCount++;
                    break;
            }
        });
...
class ValidationUtils {
     ...
     */
    static validateHealthDataPoint(point) {
        const errors = [];
     ...
     */
    static isValidHorizon(horizon) {
        return /^\d+-(day|week|month)s?$/.test(horizon);
    }
    /**
     * Sanitize metric name
     */
    static sanitizeMetricName(metric) {
        return metric
            .toLowerCase()
            .replace(/[^a-z0-9_]/g, '_')
            .replace(/_+/g, '_')
